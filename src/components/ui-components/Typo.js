...
export const Typo = styled.h2(({ h1, h3, h4, light, intro, travelQuote }) => [
  ...
  travelQuote &&
    tw`leading-relaxed text-center sm:leading-loose text-gold font-script text-f-24 sm:text-f-40`,
]);

export const TravelQuote = ({ author, children, className, ...props }) => {
  return (
    <div className={clsx("container px-5 my-5 sm:my-10", className)} {...props}>
      <Typo as="h2" travelQuote>
        {children}
      </Typo>
      <div className="text-center text-gold font-light capitalize text-f-22 sm:text-[30px] mt-5">
        {author}
      </div>
    </div>
  );
